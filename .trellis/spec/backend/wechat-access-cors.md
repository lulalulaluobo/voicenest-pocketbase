...
- Good：用户填本地 `.dev.vars`，部署代理创建独立 KV、导入 Secret、写入最终 Pages/Vercel Origin；用户在已登录 Access 的 PWA 中上传一次默认封面，然后发布草稿。
...
5. 部署后在浏览器中完成 Access 登录，运行“测试连接”、上传默认封面并验证预览；若收到 `WECHAT_IP_NOT_ALLOWED`，更新公众号 IP 白名单后再次验证。
