...
保存于新的本地配置键；参考图 Blob 只存在当前浏览器。设置页提供管理列表和独立编辑页/弹窗，支持新增、修改和删除，复用笔记分类管理的交互层级。
...
所有图片输入与输出必须限制 MIME 为 PNG/JPEG/WebP，限制大小后才进行 base64/Blob 转换。具体上限以 Agnes 冒烟测试和微信公众号永久图片素材限制为准。
