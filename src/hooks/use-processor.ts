  ...
  const processRecording = useCallback((id: string, mode: 'full' | 'sync_only' = 'full') => {
    return runSerialProcessing(id, async () => {
      ...
      try {
        ...
        if (mode === 'full') {
          await recordingDb.recordings.update(id, { status: 'processing', errorMessage: undefined, updatedAt: new Date().toISOString() })
          const chunks = await getChunks(id)
          if (!chunks.length) throw new Error('该录音没有可用的音频分片数据。')

          const transcript = await transcribeAudio(
            new Blob(chunks.map((chunk) => chunk.blob), { type: rec.mimeType }),
            getASRConfig(),
          )
          await recordingDb.recordings.update(id, { transcript, updatedAt: new Date().toISOString() })
